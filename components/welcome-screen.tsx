...
function FeatureRow({ feature }: { feature: Feature }) {
  ...
  const backgroundClass = feature.tone === 'lilac' ? 'bg-brand-lilac' : 'bg-brand-aqua';

  return (
    <View className="flex-row items-center gap-3">
      <View className={`h-10 w-10 items-center justify-center rounded-full ${backgroundClass}`}>
        <Icon color="#263238" size={21} strokeWidth={2.25} />
      </View>
      <Text className="flex-1 text-[14px] font-semibold leading-5 text-brand-ink" selectable>
        {feature.label}
      </Text>
    </View>
  );
      ...
      <ScrollView
        ...
        <View className="w-full gap-4" style={{ maxWidth: 430 }}>
          ...
          <View className="items-center gap-2">
            ...
            <Text className="text-center text-[19px] font-bold leading-[25px] text-brand-ink" selectable>
              Teman perjalanan{'\n'}pengobatanmu
            </Text>
          </View>
          ...
          <View className="gap-2.5 pt-1">
            <Pressable
              ...
              <Text className="text-[17px] font-bold text-brand-white">Masuk</Text>
            </Pressable>
            <Pressable
              accessibilityRole="button"
              className="h-12 items-center justify-center rounded-control border-2 border-brand-ink bg-brand-white"
              onPress={() => router.push('/(auth)/register')}>
              <Text className="text-[17px] font-bold text-brand-ink">Buat akun</Text>
            </Pressable>
          </View>
        </View>
      </ScrollView>
